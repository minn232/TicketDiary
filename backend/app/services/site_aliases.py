...
# 각자 다른 테이블을 유지하다 한쪽에만 별칭이 추가돼 어긋나는 걸 막기 위해 통합함. 인터파크는
# "NOL 티켓"→"NOL(야놀자)"로 예매가 이관 중(실측 확인)이나 relatenm 표기는 둘 다 "NOL"
# 계열이라 매핑은 INTERPARK 유지, 도메인 변화 대응은 crawler.py가 따로 담당.

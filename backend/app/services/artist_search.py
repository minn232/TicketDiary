...
async def search_artists(db: AsyncSession, query: str, limit: int = 30) -> list[dict]:
    ...
    # 멤버-그룹 판정을 위해 지금까지 모은 canonical들의 별칭까지 모아서, "자기 이름(canonical_name/
    # 별칭 어느 쪽이든)으로 등록된 공연이 실제로 있는지" 한 번에 확인
    ...
    for cid in canonicals:
        ...
        if group_ids is None:
            final_ids.add(cid)  # 그룹이거나 멤버 관계가 없는 아티스트 - 그대로 노출
            continue
        if has_own_concert.get(cid):
            final_ids.add(cid)  # 자기 공연 있는 멤버는 자신도 노출
        final_ids.update(group_ids)  # 자기 공연 없으면 그룹만, 있으면 그룹도 추가

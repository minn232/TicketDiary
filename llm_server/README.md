...
**포트 주의**: vLLM(OpenAI 호환 서버)이 8000번을 쓰기로 확정됐습니다(`.env`의
`VLLM_BASE_URL` 기본값도 `http://localhost:8000/v1`). 이 FastAPI 래퍼는 **8002번**으로
띄우세요 - 8001번은 이 인스턴스의 nginx가 기본으로 예약해 둔 프록시라(`/etc/nginx/nginx.conf`
참고) 쓸 수 없습니다.

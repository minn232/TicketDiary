...
- **동시 처리**: 배치로 들어온 항목들은 순차가 아니라 `.env`의 `BATCH_CONCURRENCY`
  개수만큼 동시에 모델 호출을 돌립니다(crawl/artist/diary 배치 공유 - 다 같은
  GPU를 쓰기 때문). 평소 트래픽은 기본값(3)이면 충분하고, 한 번에 몰아서 대량
  처리해야 할 때만(예: 최초 아티스트 백필) 이 값을 올려서 처리 시간을 줄일 수
  있습니다. 너무 높이면 vLLM이 GPU 메모리 부족으로 죽을 수 있으니, 낮은 값으로
  먼저 테스트해보고 점진적으로 올려주세요.

...
타임테이블 / 라인업 / 티켓 배송일 / 음식물 반입 여부를 JSON으로 추출한다.
...
3. ticket_delivery_date (티켓 배송 날짜): 이미지에 명시된 날짜가 있으면 YYYY-MM-DD로 변환합니다. 배송 관련 언급이 전혀 없으면 null로 둡니다.
4. ticket_prices (이용권/좌석별 가격): 매우 보수적으로 채우세요. 이미지 안 한 곳에 "구분 명칭"과 "정확한 금액(원)"이 서로 붙어 나란히 인쇄되어 있는 것을 실제로 두 눈으로 읽었을 때만 항목을 만들고, 그 글자와 숫자를 있는 그대로 seat_type과 price(정수, 원 단위)로 옮겨 적으세요.
...
5. other_info.food_allowed (음식물 반입 가능 여부): "반입 가능 물품"/"반입 금지 물품" 아이콘 안내가 있는지 먼저 확인하세요(예: "500ml 이하 페트병/텀블러 음료는 가능, 병/캔/유리 용기·500ml 초과 음료·도시락 등은 금지" 처럼 아이콘과 짧은 설명으로 표시되는 경우가 많습니다).

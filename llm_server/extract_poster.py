...
가수/팀 이름(timetable, lineup의 artist 필드)은 특히 주의하세요 - 이미지 안에 실제로 인쇄되거나
표시된 글자만 쓰고, 인물의 얼굴/스타일/분위기를 보고 당신이 이미 알고 있는 실제 이름(본명 등)이나
배경지식으로 답하지 마세요. 이미지에 활동명만 쓰여 있으면 활동명만 답하고, 이미지에 없는 본명으로
바꿔 쓰지 마세요 - 그 이름이 텍스트로 쓰여 있지 않다면 null로 둡니다.
